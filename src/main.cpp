...
void initialize() {
  ...
  chassis.opcontrol_curve_default_set(2, 5);  // Defaults for curve. If using tank, only the first parameter is used. (Comment this line out if you have an SD card!)
  ...
  ez::as::auton_selector.autons_add({
      {"Red Side ",odom_boomerang_example},
      {"Blue Side",odom_boomerang_example},
      {"Skills",odom_boomerang_example},
...
void ez_template_extras() {
  ...
  //   autonomous();
  // }
  // if(master.get_digital(DIGITAL_L1)){ // turns of antijam when scoring in middle goal
  //   setAntiJam(true);
  // }

  // if (master.get_digital(DIGITAL_DOWN)){ // park and low goal
  ...
  //   score.move(-127*0.85);
  //   low.move(-127*0.65);
  //   pros::delay(1000); // Waiting for it to score

  //   chassis.pid_turn_set(15_deg, TURN_SPEED, false); // turn to face park
  //   chassis.pid_wait_quick();
  //   chassis.odom_xyt_set(0_in, 0_in, 0_deg);
    
  //   chassis.pid_drive_set(-50_in, DRIVE_SPEED, true); // drive into park
  //   pros::delay(1030);

  //   chassis.pid_turn_set(-68_deg, TURN_SPEED, false); // turn to face park
  //   chassis.pid_wait_quick();
  //   chassis.odom_xyt_set(0_in, 0_in, 0_deg);

  //   chassis.pid_drive_set(17_in, DRIVE_SPEED, true); // drive into park
  //   chassis.pid_wait_quick();
  // }
  // else if (master.get_digital(DIGITAL_UP)){ // just park
  //   chassis.pid_drive_set(24_in, DRIVE_SPEED, true); // drive out of goal
  //   chassis.pid_wait_quick_chain();

  //   chassis.pid_turn_set(71_deg, TURN_SPEED, false); // turn to face park
  //   chassis.pid_wait_quick_chain();

  //   chassis.pid_drive_set(30_in, DRIVE_SPEED, true); // drive into park
  //   chassis.pid_wait_quick_chain();
  // }
}
...
void opcontrol() {
  ...
  while (true) {
    ...
    //   }
    //   if (!killSwitchOn) {
    //     chassis.pid_turn_set(134_deg, TURN_SPEED, false); // turn to face 1st trio ball
    //     chassis.pid_wait();
    //     chassis.odom_xyt_set(0_in, 0_in, 0_deg);
    //   }
    ...
    //   if (!killSwitchOn) {
    //     chassis.pid_drive_set(48_in, DRIVE_SPEED, true); // drive into trio balls and low goal
    //     chassis.pid_wait_until(10);
    //     chassis.pid_speed_max_set(DRIVE_SPEED*0.65); 
    //     chassis.pid_wait_quick();
    //     chassis.odom_xyt_set(0_in, 0_in, 0_deg);
    //   }
    ...
    //     chassis.pid_odom_set({{26.5_in, 36.5_in, 90_deg}, fwd, 70});
    //     chassis.pid_wait();
    //   }
